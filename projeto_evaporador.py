...
def main(T_in, T_ref, uf, W, H, L1, L2, D, ST, SL, NT, NL1, NL2, t_fin, N_fin1, N_fin2):
    ...
    termodynamic_values = pd.DataFrame(data={
            ...
            'Taxa de troca de calor (Q) [W]': [round(Q, 3), round(Q1, 3), round(Q2, 3)]}, index = ['Total', '1ª Parte', '2ª Parte']).T

    fluidynamic_values = pd.DataFrame(data={
            'Perda de Carga (ΔP) [Pa]': [round(P, 3), round(P1, 3), round(P2, 3)],
            'Potência Mecânica (W) [W]': [W, '---', '---']}, index = ['Total', '1ª Parte', '2ª Parte']).T
    ...
    return geometric_values, termodynamic_values, fluidynamic_values, entropy_values, optimize_values, post_optimize_values
...
if st.button('EXECUTAR'):
    geometric_values, termodynamic_values, fluidynamic_values, entropy_values, optimize_values, post_optimize_values = main(T_in, T_ref, uf, W, H, L1, L2, D, ST, SL, NT, NL1, NL2, t_fin, N_fin1, N_fin2)
    print('\nRESULTADOS GEOMÉTRICOS:\n')
    print(pd.DataFrame(geometric_values))
    print('\nRESULTADOS TÉRMICOS:\n')
    print(pd.DataFrame(termodynamic_values))
    print('\nRESULTADOS FLUIDODINÂMICOS:\n')
    print(pd.DataFrame(fluidynamic_values))
    print('\nRESULTADOS DE ENTROPIA:\n')
    print(pd.DataFrame(entropy_values))
    print('\nANÁLISE DE OTIMIZAÇÃO:\n')
    print(pd.DataFrame(optimize_values))
    print(('\nDADOS DE OTIMIZAÇÃO BASEADOS EM T_ref_otm2:\n'))
    print(pd.DataFrame(post_optimize_values))
    ...
    with col2:
        ...
        st.write('RESULTADOS FLUIDODINÂMICOS:')
        st.write(fluidynamic_values)
